...
test('createPurchase accepts items without productId and computes total', async () => {
  const p = await purchaseService.createPurchase('u-no-prod', [
    { name: 'DirectItem', price: 4.5, quantity: 3 },
  ])

  expect(p.items[0].name).toBe('DirectItem')
  expect(p.items[0].price).toBe(4.5)
  expect(p.total).toBeCloseTo(13.5)
})

test('listPurchases with only from filter returns expected docs', async () => {
  const base = new Date('2025-03-01T00:00:00.000Z')
  const p1 = await Purchase.create({ userId: 'u2', items: [{ name: 'A', price: 1, quantity: 1 }], total: 1, createdAt: base })
  const p2 = await Purchase.create({ userId: 'u2', items: [{ name: 'B', price: 2, quantity: 1 }], total: 2, createdAt: new Date('2025-03-10T00:00:00.000Z') })
